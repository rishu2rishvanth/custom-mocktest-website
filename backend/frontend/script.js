...
function populateSections() {
  fetch('http://10.10.182.4:5000/api/questions/sections')
    .then(res => res.json())
    .then(data => {
...
function showNextQuestion() {
  ...
  if (current['Question Image URL']) {
    ...
    img.src = `http://10.10.182.4:5000${current['Question Image URL']}`;
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (imgUrl) {
      const img = document.createElement('img');
      img.src = `http://10.10.182.4:5000${imgUrl}`;
...
function handleAnswer(index, button) {
  ...
  const img = current[`Answer ${index + 1} Image URL`]
    ? `http://10.10.182.4:5000${current[`Answer ${index + 1} Image URL`]}`
    : '';
...
function submitResponses() {
  ...
  fetch('http://10.10.182.4:5000/api/response', {
    method: 'POST',
  ...
  fetch('http://10.10.182.4:5000/api/score', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
